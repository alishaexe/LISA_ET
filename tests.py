...
def pls(f):
    ...
    res = om * (f/fst)**(n)
    return res
plsle = np.array(list(map(pls, fvalscomb)))
...
plt.plot(fvalscomb, plsle, color = "black")
...
et = np.load('/Users/alisha/Documents/LISA_ET/LISA/ftabET.npy')
lisa = np.load('/Users/alisha/Documents/LISA_ET/LISA/ftablisa.npy')
comb = np.load("/Users/alisha/Documents/LISA_ET/LISA/Ftabbigsigcomb.npy")
plt.figure(figsize=(6, 9))
plt.loglog(otog[:,0], nom , color = "indigo", label = "Nominal", linewidth=2.5)
plt.plot(fvalscomb,bplet)
plt.plot(np.exp(et[:,0]), np.exp(et[:,1]))
plt.plot(np.exp(lisa[:,0]),np.exp(lisa[:,1]))
plt.plot(np.exp(comb[:,0]),np.exp(comb[:,1]))
plt.ylim(1e-14,1e-5)
plt.title("Nominal and PLS curves ", fontsize = 16)
# plt.legend(loc = (1.05,0.5), fontsize = 14)
plt.grid(True) 
