...
plt.loglog(floglisa[:,0][floglisa[:,0]<1e-1], floglisa[:,1][floglisa[:,0]<1e-1],':', color = "teal", label = "LISA LNS", linewidth=2.5)
plt.loglog(floget[:,0][floget[:,0]>1e0], floget[:,1][floget[:,0]>1e0],':', color = "black", linewidth=2.5, label= "ET LNS")
plt.legend(loc=2,fontsize = 18)
plt.tick_params(axis='both', which='major', labelsize=14)
