...
def f01(f0, nt, omegstar):
    integrand = lambda f, f0, nt, omegstar: omegstar*(f/f0)**(2*nt)*log(f/f0)/SigmaLisaApprox(f)
    res = quad(integrand, 1e-5, 1e-1, args=( f0, nt, omegstar))[0]
    return res

def f11(f0, nt, omegstar):
    integrand = lambda f, f0, nt, omegstar: omegstar**2*(f/f0)**(2*nt)*log(f/f0)**2/SigmaLisaApprox(f)
    res = quad(integrand, 1e-5, 1e-1, args=( f0, nt, omegstar))[0]
    return res
...
def Fisher(differential):
    ...
    res = integrate(integrand(f), (f, 0.01, 10**(-1)))
...
def FisherB(differential):
    os = 10**(-9)
    ntv = 0.01
    f0v = 0.1
    integrand = lambda f: differential.subs({omegstar: os, nt: ntv, f0: f0v})/SigmaLisaApprox(f)**2
    print('after integrand', differential)
    res = integrate(integrand(f), (f, 10**(-5), 10**(-1)))
    print('finished this int', differential)
    return res
# 10**(-5)
def FisherETB(differential):
    os = 10**(-9)
    ntv = 0.01
    f0v = 0.1
