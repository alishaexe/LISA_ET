...
fs = 10**elbpl
inputs = np.array(np.meshgrid(n2r, sigma, n1r, fs)).T.reshape(-1,4)
#This makes it so n1r is in the second column
# so inputs(fs, n1r, n2r)
inputs[:,[0,1,2,3]] = inputs[:,[3,2,0,1]]
...
fLvals = 10**elbpl
coords = np.array(np.meshgrid(k,m,j,i, fLvals)).T.reshape(-1,5)
...
inputsET = np.array(np.meshgrid(n2r, sigma, n1r, fs)).T.reshape(-1,4)
#This makes it so n1r is in the second column
# so inputs(fs, n1r, n2r)
inputsET[:,[0,1,2,3]] = inputsET[:,[3,2,0,1]]
#%%
...
m = range(len(sigma))
fvals = (10**elET)
coords = np.array(np.meshgrid(k, m, j, i, fvals)).T.reshape(-1,5)
coords[:,[0,1,2,3,4]] = coords[:,[4,3,2,0,1]]
#%%
