...
def sigETlapp(f):#Sigma_Ohm approx
    if f <= 0.295 or f >= 500:
        return 10**(-5)        
    if f > 0.295 and f < 500:
        res = siglp(f)
        return res
...
def AlogETmin(fstar, sig):
    integrand = lambda f, fstar, sig :(logn(f, fstar, sig)/sigETlapp(f))**2
    I1 = quad(integrand, ffmin, 10**(-4), args=(fstar, sig))[0]
    I2 = quad(integrand, 10**(-4), 10**(-2), args=(fstar, sig))[0]
    I3 = quad(integrand, 10**(-2), 10**(-1), args=(fstar, sig))[0]
    I4 = quad(integrand, 10**(-1), 10, args=(fstar, sig))[0]
    I5 = quad(integrand, 10, ffmax, args=(fstar, sig))[0]
    return snr5/np.sqrt(T*(I1+I2+I3+I4+I5))
lf = np.linspace(elmin, elmax, itera)
flf = 10**lf
...
plt.loglog(np.exp(np.log(flf)), np.exp(maxlog), color = "aqua")
plt.title("LogNS curve")
plt.ylim(1e-14, 1e-1)
plt.xlim(ffmin, ffmax)
plt.xlabel('f (Hz)')
plt.ylabel('Omega_gw')
...
def aminlogL(fstar, sig):
    integrand = lambda f, fstar, sig: (lognL(f, fstar, sig)/sigLlogapprox(f))**2
    I1 = quad(integrand, ffmin, 10**(-4), args=(fstar, sig))[0]
    I2 = quad(integrand, 10**(-4), 10**(-2), args=(fstar, sig))[0]
    I3 = quad(integrand, 10**(-2), 10**(-1), args=(fstar, sig))[0]
    I4 = quad(integrand, 10**(-1), 10, args=(fstar, sig))[0]
...
ls = np.linspace(elmin, elmax,itera)
fls = 10**ls
sigma = np.linspace(-1, 0, 5)
...
maxlogL = np.array(list(map(maxlogvalsL, maxposlog)))
...
def aminlogcomb(fstar, sig):
    integrand = lambda f, fstar, sig: (logncomb(f, fstar, sig)/omegalog(f))**2
    I1 = quad(integrand, ffmin, 10**(-4), args=(fstar, sig))[0]
    I2 = quad(integrand, 10**(-4), 10**(-2), args=(fstar, sig))[0]
    I3 = quad(integrand, 10**(-2), 10**(-1), args=(fstar, sig))[0]
    I4 = quad(integrand, 10**(-1), 10, args=(fstar, sig))[0]
...
valslog = np.array(np.meshgrid(fls, sigma)).T.reshape(-1,2)
aminvalsc = np.array(list(map(lambda args: aminlogcomb(*args), valslog)))

atablogc = np.vstack((valslog.T, aminvalsc)).T.reshape(-1,len(sigma), 3)
...
Ftablogcomb = np.array(list(map(lambda args: fLlogtabc(*args), coords))).reshape(len(fls),len(fls),len(sigma))
...
maxlogc = np.array(list(map(maxlogvalsc, maxposlog)))
flogplotL = np.vstack((np.log(fls), maxlogL)).T

plt.loglog(np.exp(np.log(fls)), np.exp(maxlogc), color = "aqua", label = "LogNs")
